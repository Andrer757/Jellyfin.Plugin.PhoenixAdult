...
{
    public class SiteVRHush : IProviderBase
    {
        ...
        foreach (var sceneURL in googleResults)
        {
            ...
            if (http.IsOK)
            {
                ...
                var titleNode = doc.DocumentNode.SelectSingleNode(@"//h1[@class=""latest-scene-title""]");
        ...
        movie.Name = doc.DocumentNode.SelectSingleNode(@"//h1[@class=""latest-scene-title""]")?.InnerText.Trim();
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//span[contains(@class, ""full-description"")]")?.InnerText.Trim();
        movie.AddStudio("VRHush");

        var dateNode = doc.DocumentNode.SelectSingleNode(@"//div[contains(@class, ""latest-scene-meta"")]//div[contains(@class, ""text-left"")]");
