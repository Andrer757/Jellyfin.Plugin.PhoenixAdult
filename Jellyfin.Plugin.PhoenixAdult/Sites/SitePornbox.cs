    ...
    public class SitePornbox : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (searchHttp.IsOK)
            {
                ...
                {
                    var titleNoFormatting = Helper.ParseTitle((string)searchResult["scene_name"]);
                    var match = Regex.Match(titleNoFormatting, @"\w+\d$");
                    if (match.Success)
                    {
                        var matchID = match.Value;
                        titleNoFormatting = Regex.Replace(titleNoFormatting, @"\w+\d$", "").Trim();
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = Helper.ParseTitle((string)json["scene_name"]);
            movie.Overview = (string)json["small_description"];
            movie.AddStudio("Pornbox");

            var tagline = Helper.ParseTitle((string)json["studio"]);
            ...
            if (json["models"] != null) actors.AddRange(json["models"]);
            if (json["male_models"] != null) actors.AddRange(json["male_models"]);
            ...
            if (tagline == "Giorgio Grandi" || tagline.Contains("Giorgio's Lab"))
                result.AddPerson(new PersonInfo { Name = "Giorgio Grandi", Type = PersonType.Director });
