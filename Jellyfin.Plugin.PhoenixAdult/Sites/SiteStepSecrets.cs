...
{
    public class SiteStepSecrets : IProviderBase
    {
        ...
        foreach (var sceneURL in googleResults)
        {
            ...
            if (http.IsOK)
            {
                ...
                var titleNode = doc.DocumentNode.SelectSingleNode(@"//h1[@class=""font-cond""]");
        ...
        movie.Name = doc.DocumentNode.SelectSingleNode(@"//h1[@class=""font-cond""]")?.InnerText.Trim();
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//div[@class=""descripton""]")?.InnerText.Trim();
        movie.AddStudio("Joymii");
