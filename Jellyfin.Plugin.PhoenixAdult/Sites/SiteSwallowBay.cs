...
{
    public class SiteSwallowBay : IProviderBase
    {
        ...
        foreach (var sceneURL in googleResults)
        {
            ...
            if (http.IsOK)
            {
                ...
                var titleNode = doc.DocumentNode.SelectSingleNode(@"//meta[@name=""twitter:image:alt""]/@content");
        ...
        movie.Name = doc.DocumentNode.SelectSingleNode(@"//meta[@name=""twitter:image:alt""]/@content")?.InnerText.Trim();
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//div[@class=""content-desc more-desc""]")?.InnerText.Trim();
        movie.AddStudio("Swallow Bay");

        var dateNode = doc.DocumentNode.SelectSingleNode(@"//div//div[@class=""content-date""]");
