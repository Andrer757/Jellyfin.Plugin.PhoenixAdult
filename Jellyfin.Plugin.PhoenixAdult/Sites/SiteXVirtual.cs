    ...
    public class SiteXVirtual : IProviderBase
    {
        ...
        foreach (var sceneURL in googleResults)
        {
            ...
            if (http.IsOK)
            {
                ...
                var titleNode = doc.DocumentNode.SelectSingleNode(@"//div[contains(@class, ""title"")]/h2");
        ...
        movie.Name = doc.DocumentNode.SelectSingleNode(@"//div[contains(@class, ""title"")]/h2")?.InnerText.Trim();
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//div[@class=""description""]/div[@class=""desc-text""]")?.InnerText.Trim();
