...
{
    public class SiteTrueAmateurs : IProviderBase
    {
        ...
        foreach (var sceneURL in googleResults)
        {
            ...
            if (http.IsOK)
            {
                ...
                var titleNode = doc.DocumentNode.SelectSingleNode(@"//h2[@class=""wxt7nk-4 fSsARZ""]");
        ...
        movie.Name = doc.DocumentNode.SelectSingleNode(@"//h2[@class=""wxt7nk-4 fSsARZ""]")?.InnerText.Trim();
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//p")?.InnerText.Trim();
        movie.AddStudio("TrueAmateurs");

        var dateNode = doc.DocumentNode.SelectSingleNode(@"//div[@class=""tjb798-3 gFvmpb""]/span[last()]");
