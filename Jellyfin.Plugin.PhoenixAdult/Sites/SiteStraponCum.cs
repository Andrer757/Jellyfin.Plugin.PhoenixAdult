    ...
    public class SiteStraponCum : IProviderBase
    {
        ...
        foreach (var sceneURL in googleResults)
        {
            ...
            if (http.IsOK)
            {
                ...
                var titleNode = doc.DocumentNode.SelectSingleNode(@"//h1[@class=""card-title""]");
        ...
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//p[@class=""card-text mb-2""]")?.InnerText.Trim();
        movie.AddStudio("Strapon Cum");

        var dateNode = doc.DocumentNode.SelectSingleNode(@"//div[@class=""card""]//i[contains(@class, ""fa-clock"")]/following-sibling::text()");
