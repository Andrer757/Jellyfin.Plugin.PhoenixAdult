...
namespace PhoenixAdult.Sites
{
    public class SitePlumperPass : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            foreach (var sceneURL in searchResults)
            {
                ...
                if (http.IsOK && http.Content.Contains("content"))
                {
                    ...
                    var titleNoFormatting = doc.DocumentNode.SelectSingleNode("//h2[@class='vidtitle']").InnerText.Trim().Replace("\"", "");
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = Helper.ParseTitle(doc.DocumentNode.SelectSingleNode("//h2[@class='vidtitle']").InnerText.Trim().Replace("\"", ""));
            movie.Overview = doc.DocumentNode.SelectSingleNode("//div[contains(@class, 'vidinfo')]/p").InnerText.Trim();
            movie.AddStudio("PlumperPass");

            var url = http.ResponseUrl;
            if (url.Contains("bbwd/")) movie.AddCollection("BBW Dreams");
            else if (url.Contains("bbbj/")) movie.AddCollection("Big Babe Blowjobs");
            else if (url.Contains("hsp/")) movie.AddCollection("Hot Sexy Plumpers");
            else if (url.Contains("patp/")) movie.AddCollection("Plumpers At Play");
            else if (url.Contains("ftf/")) movie.AddCollection("First Time Fatties");
            else if (url.Contains("bgb/")) movie.AddCollection("BBWs Gone Black");
            else movie.AddCollection("PlumperPass");
            ...
            if (actors != null)
            {
                ...
                foreach (var actorLink in actors)
                {
                    ...
                    if (actorHttp.IsOK)
                    {
                        ...
                        result.AddPerson(new PersonInfo { Name = actorName, Type = PersonType.Actor, ImageUrl = actorPhotoURL });
                    }
                }
            }
