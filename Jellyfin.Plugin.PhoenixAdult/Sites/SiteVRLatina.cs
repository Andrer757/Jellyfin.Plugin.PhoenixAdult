...
{
    public class SiteVRLatina : IProviderBase
    {
        ...
        movie.Name = doc.DocumentNode.SelectSingleNode(@"//h2")?.InnerText.Trim();
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//div[@class=""content-desc""]")?.InnerText.Trim();
        movie.AddStudio("Unknown");

        var dateNode = doc.DocumentNode.SelectSingleNode(@"//div[@class=""content-base-info""]//div[@class=""info-elem -length""]/span");
