...
namespace PhoenixAdult.Sites
{
    public class SitePenthouseGold : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            if (http.IsOK)
            {
                ...
                foreach (var searchResult in doc.DocumentNode.SelectNodes("//div[@class='scene']"))
                {
                    ...
                    if (!url.Contains("/scenes/")) continue;
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = doc.DocumentNode.SelectSingleNode("//div[@class='content-desc content-new-scene']//h1").InnerText.Replace("Video -", "").Replace("Movie -", "").Trim();
