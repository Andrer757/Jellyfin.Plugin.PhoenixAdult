...
namespace PhoenixAdult.Sites
{
    public class SiteMyDirtyHobby : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            var http = await HTTP.Request(searchUrl, cancellationToken, null, null, "POST", searchBody, new Dictionary<string, string> { { "Content-Type", "application/json" } });
            if (http.IsOK)
            {
                ...
                if (items != null)
                {
                    ...
                    {
                        if ((string)item["contentType"] != "video") continue;
                        ...
                        result.Add(new RemoteSearchResult
                        {
                            ...
                            Name = $"{Helper.ParseTitle(titleNoFormatting)} [{Helper.GetSearchSiteName(siteNum)}/{userNickname}] {releaseDate}",
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = Helper.ParseTitle((string)videoPageElements.SelectToken("title.text")).Trim();
            ...
            result.AddPerson(new PersonInfo { Name = actorName.Trim(), Type = PersonType.Actor, ImageUrl = actorPhotoURL });
