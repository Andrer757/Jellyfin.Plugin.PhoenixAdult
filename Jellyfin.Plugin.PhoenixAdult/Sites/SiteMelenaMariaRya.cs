...
namespace PhoenixAdult.Sites
{
    public class SiteMelenaMariaRya : IProviderBase
    {
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            title = Regex.Replace(title, @"\s+4\s*K(?:\s+Video)?$", "", RegexOptions.IgnoreCase);
            movie.Name = title;
            ...
            result.AddPerson(new PersonInfo { Name = "Melena Maria Rya", Type = PersonType.Actor });

            var match = Regex.Match(title, @" with ([A-Z][a-z]+ [A-Z][a-z]+)$", RegexOptions.IgnoreCase);
            if (match.Success)
            {
                result.AddPerson(new PersonInfo { Name = match.Groups[1].Value, Type = PersonType.Actor });
            }
