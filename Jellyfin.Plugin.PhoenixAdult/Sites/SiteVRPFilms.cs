...
{
    public class SiteVRPFilms : IProviderBase
    {
        ...
        foreach (var sceneURL in googleResults)
        {
            ...
            if (http.IsOK)
            {
                ...
                var titleNode = doc.DocumentNode.SelectSingleNode(@"//section[@class=""login-banner parallax""]/h1");
        ...
        movie.Name = doc.DocumentNode.SelectSingleNode(@"//section[@class=""login-banner parallax""]/h1")?.InnerText.Trim();
        movie.Overview = doc.DocumentNode.SelectSingleNode(@"//div[@class=""col-md-8 text-justify""]")?.InnerText.Trim();
