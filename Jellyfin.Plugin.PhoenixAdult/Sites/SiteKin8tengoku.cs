...
namespace PhoenixAdult.Sites
{
    public class SiteKin8tengoku : IProviderBase
    {
        public async Task<List<RemoteSearchResult>> Search(int[] siteNum, string searchTitle, DateTime? searchDate, CancellationToken cancellationToken)
        {
            ...
            var searchTitleNoID = string.IsNullOrEmpty(sceneID) ? searchTitle : searchTitle.Replace(sceneID, "").Trim();
            ...
            if (!string.IsNullOrEmpty(sceneID))
            {
                ...
                if (http.IsOK)
                {
                    ...
                    var titleNoFormatting = titleNode != null ? Helper.ParseTitle(titleNode.InnerText.Split('/')[0].Trim()) : string.Empty;
            ...
            if (searchHttp.IsOK)
            {
                ...
                foreach (var searchResult in doc.DocumentNode.SelectNodes("//div[@class='movie_list']"))
                {
                    ...
                    var titleNoFormatting = Helper.ParseTitle(searchResult.SelectSingleNode(".//div[@class='movielisttext02']").InnerText.Trim());
                    var curID = Helper.Encode(sceneURL);
        ...
        public async Task<MetadataResult<BaseItem>> Update(int[] siteNum, string[] sceneID, CancellationToken cancellationToken)
        {
            ...
            movie.Name = Helper.ParseTitle(doc.DocumentNode.SelectSingleNode("//p[@class='sub_title']|//p[@class='sub_title_vip']").InnerText.Split('/')[0].Trim());
